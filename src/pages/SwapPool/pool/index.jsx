...
const Pool = () => {
  ...
  const getUserLiquidity = async () => {
    ...
    const ORU_USDC_USER =
      (oruUsdcLiq / (+(await ORU_USDC.totalSupply()) / 1e18)) *
      (+(await ORU_USDC.balanceOf(account)) / 1e18);

    const OUSD_USDC_USER =
      (ousdUsdcLiq / (+(await OUSD_USDC.totalSupply()) / 1e18)) *
      (+(await OUSD_USDC.balanceOf(account)) / 1e18);
    const ORU_OUSD_USER =
      (oruOusdLiq / (+(await OUSD_ORU.totalSupply()) / 1e18)) *
      (+(await OUSD_ORU.balanceOf(account)) / 1e18);
  ...
  const getUserLPBalance = async () => {
    const pool =
      contracts[selectedPool.token0.name + "_" + selectedPool.token1.name] ||
      contracts[selectedPool.token1.name + "_" + selectedPool.token0.name];
  ...
  const addLiquidity = async () => {
    ...
    try {
      const tx = await ROUTER.connect(signer).addLiquidity(
        ...
        (+formatToDecimal(
          token1Input,
          selectedPool.token1.name === "USDC" ? 6 : 18
        )).toFixed(0),
        0,
        0,
        account,
      ...
      <PoolBlockWrapper height={isRemoveLiquidity ? "20vw" : ""}>
        {isAddLiquidity || isRemoveLiquidity ? (
          <>
            ...
            >
              ...
              >
                ...
                <b>
                  {isRemoveLiquidity ? "Remove Liquidity" : "Add Liquidity"}
                </b>
              </div>
              {pools && selectedPool ? (
                <PseudoPoolSelect
          ...
          <>
            ...
            <RemoveLiqBtn onClick={removeLiquidityWindowHandler}>
              Remove Liquidity
            </RemoveLiqBtn>
            ...
            <OrangeBlock>
              {userLiq ? (
                <>
                  <OrangeRow>
                    <div>ORU/USDC</div>
                    <div>{formattedNum(userLiq.ORU_USDC_USER)}</div>{" "}
                  </OrangeRow>
                  <OrangeRow>
                    <div>OUSD/USDC</div>
                    <div>{formattedNum(userLiq.OUSD_USDC_USER)} </div>
                  </OrangeRow>
                  <OrangeRow>
                    <div> ORU/OUSD</div>
                    {/* <LogoIcon color={"black"}/><OUSDIcon color={"black"}/> */}
                    <div> {formattedNum(userLiq.ORU_OUSD_USER)} </div>
                  </OrangeRow>
                </>
              ) : (
