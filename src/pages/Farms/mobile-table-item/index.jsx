...
const MobileTableItm = ({ index, item }) => {
  ...
  const getPoolInfo = async () => {
    ...
    const lpBalance =
      +(await lpToken.balanceOf(CONTRACT_ADDRESSES.MASTER_CHEF)) / 1e18;
    const poolInfo = await MASTER_CHEF.poolInfo(index);

    console.log(poolInfo);

    const apr = (
      ((liquidity.oruPrice * (ORU_PER_BLOCK / 3) * 86400 * 30 * 12) /
        2 /
        (lpPrice * lpBalance)) *
      100
    ).toFixed(0);
  ...
  const getUserInfo = async () => {
    ...
    const pendingReward =
      +(await MASTER_CHEF.pendingOru(index, account)) / 1e18;

    const locked =
      +userContractInfo.depositTime - +poolInfo.lockDuration >=
  ...
  const handleApprove = async () => {
    try {
      ...
      const tx = await lpToken
        .connect(signer)
        .approve(CONTRACT_ADDRESSES.MASTER_CHEF, MAX_INT);
  ...
  const handleSlider = (isDeposit, value) => {
    if (isDeposit) {
      const depositAmt = (
        userInfo.lpBalance * PERCENTAGES[value] -
        userInfo.lpBalance * PERCENTAGES[value] * 0.00005
      ).toFixed(0);
      setDepositInput(depositAmt);
      setDepositSliderPosition(value);
    } else {
      const withdrawAmt = (
        userInfo.depositedAmt * PERCENTAGES[value] -
        userInfo.depositedAmt * PERCENTAGES[value] * 0.00005
      ).toFixed(0);
  ...
  const handleDeposit = async () => {
    ...
    try {
      const tx = await MASTER_CHEF.connect(signer).deposit(
        index,
        ethers.BigNumber.from(fromExponential(depositInput)),
        account
      );
  ...
  const handleWithdraw = async () => {
    ...
    try {
      const tx = await MASTER_CHEF.connect(signer).withdraw(
        index,
        ethers.BigNumber.from(fromExponential(withdrawInput))
      );
      await tx.wait();
      await getUserInfo();
    } catch (e) {
        ...
        <>
          <AdditionalExpanded>
            ...
            <AdditionalRow>
              ...
              <div>
                <b>
                  ...
                  {poolInfo && userInfo
                    ? formattedNum(
                        poolInfo.lpPrice * (userInfo.depositedAmt / 1e18)
                      )
                    : 0}
                </b>
              </div>
            ...
            <AdditionalRow>
              <div>Rates</div>
              <div>
                APR <b>{poolInfo ? formattedNum(poolInfo.apr) : 0}%</b>
              </div>
            </AdditionalRow>
          </AdditionalExpanded>
          <ExpandedDataWrapper>
            <ExpandedData>
              <VDiv w="100%">
                <HDiv>
                  <Text color="#4F4F4F" fW="300" ml="0.833vw">
                    Balance:{" "}
                    {userInfo ? formattedNum(userInfo.lpBalance / 1e18) : 0.0}{" "}
                    {item.name}{" "}
                  </Text>
                  <Text>
                    <b></b>
                ...
                >
                  <OutlineBtn
                    gap="12px"
                    onClick={() => proxyNavigation(item.addURL)}
                    width={isMobileScreen() ? "48%" : "15.8vw"}
                  >
                    <PlusIcon
                  ...
                  <OutlineBtn
                    gap="12px"
                    onClick={() => proxyNavigation(item.removeURL)}
                    width={isMobileScreen() ? "48%" : "15.8vw"}
                  >
                    <TrashIcon
                      ratio={isMobileScreen() ? "5vw" : undefined}
              ...
              <VDiv w="100%" style={{ marginTop: "24px" }}>
                <HDiv>
                  <Text ml="0.833vw">
                    Balance:{" "}
                    {userInfo
                      ? fromExponential(userInfo.depositedAmt / 1e18)
                      : 0.0}
                    &nbsp;{item.name}{" "}
                  </Text>
                  <Text></Text>
                ...
                >
                  <div style={{ display: "flex", gap: "4px" }}>
                    <HelpCircleContainer>
                      ...
                      <HelpText>
                        To start vesting rewards please click on Start Vesting
                        at first, then you will be eligible to claim
                        <br />
                        <br />
                    ...
                    <Text color="#333" fw="500" fontSize="12px" mr="0.573vw">
                      Pending Rewards:
                      <br />{" "}
                      {userInfo ? formattedNum(userInfo.pendingReward) : 0} ORU
                    </Text>
                  </div>
