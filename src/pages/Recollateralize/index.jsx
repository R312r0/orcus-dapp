...
const Recollateralize = () => {
  ...
  const getUserInfo = async () => {
    ...
    const allowance =
      (await USDC.allowance(account, CONTRACT_ADDRESSES.BANK)) > 0;
  ...
  const handleRecollat = async () => {
    ...
    try {
      const tx = await BANK.connect(signer).recollateralize(
        formatToDecimal(collatInput, 6),
        0
      );
  ...
  const RecollatButton = () => {
    if (account && signer) {
      if (info?.recollatAvailable === 0) {
        return (
          <RecollateralizeBtn disabled={true}>
            Recollaterize unavailable
          </RecollateralizeBtn>
        );
      } else if (!userInfo?.allowance) {
        return (
          <RecollateralizeBtn onClick={() => handleApprove()}>
            Approve USDC
          </RecollateralizeBtn>
        );
        ...
        return (
          <RecollateralizeBtn onClick={() => handleRecollat()} disabled={false}>
            Insufficient recollat amount
          </RecollateralizeBtn>
        );
      } else {
        return (
          <RecollateralizeBtn onClick={() => handleRecollat()}>
            Recolaterize
          </RecollateralizeBtn>
        );
      }
      ...
      return (
        <RecollateralizeBtn onClick={() => connectWallet()}>
          Connect Wallet
        </RecollateralizeBtn>
      );
    }
    ...
    <RecollateralizeWrapper>
      <StakingWrapper>
        ...
        <TabWrapper>
          <RecollateralizeBlockWrapper>
            <HDiv mt="1.094vw">
              <Text fS="14px" fontWeight="500">
                Recollaterize available
              </Text>
              <Text>Balance:</Text>
            </HDiv>
            <HDiv mt={isMobileScreen() ? "24px" : "1.094vw"}>
              <Text>
                {info ? formattedNum(info.recollatAvailable) : 0.0} USDC
              </Text>
            ...
            <RecollateralizeInputWrapper withBtn>
              <input
                ...
                placeholder="0.0"
                value={collatInput}
                onChange={({ target }) => handleCollatInput(target.value)}
              />
              <button
                onClick={() =>
                  userInfo ? handleCollatInput(userInfo.bal) : null
                }
              >
                Max
              </button>
            ...
            <HDiv>
              ...
              <Text>
                Balance: {userInfo ? formattedNum(userInfo.oruBal) : 0}
              </Text>
            </HDiv>
            <RecollateralizeInputWrapper>
          ...
          <RecollateralizeDataWrapper>
            ...
            <HDiv h="50px">
              <div style={{ display: "flex", alignItems: "center" }}>
                <RecollateralizeDataText mr="0.339vw">
                  Collateral balance
                </RecollateralizeDataText>
