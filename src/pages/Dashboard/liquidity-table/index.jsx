...
const LiquidityTable = () => {
  ...
  const getProtocolLiquidity = async () => {
    ...
    const protocolOwned =
      (+(await ORU_USDC.balanceOf(CONTRACT_ADDRESSES.TREASURY)) / 1e18) *
      lpPrice;
    const protocolRented =
      (+(await ORU_USDC.balanceOf(CONTRACT_ADDRESSES.DUSTBIN)) / 1e18) *
      lpPrice;
        ...
        <VDiv>
          ...
          <Text mt={isMobileScreen() ? "10px" : "0.156vw"}>
            <b>
              $
              {protocolLiquidity
                ? formattedNum(protocolLiquidity.protocolOwned)
                : 0}
            </b>
          </Text>
        </VDiv>
        ...
        <VDiv>
          ...
          <Text mt={isMobileScreen() ? "10px" : "0.156vw"}>
            <b>
              $
              {protocolLiquidity
                ? formattedNum(protocolLiquidity.protocolRented)
                : 0}
            </b>
          </Text>
        </VDiv>
