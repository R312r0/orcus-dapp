...
const Redeem = () => {
  ...
  const getUserInfo = async () => {
    ...
    const ousdAllowance =
      (await OUSD.allowance(account, CONTRACT_ADDRESSES.BANK)) > 0;
  ...
  const handleStableInput = (value) => {
    ...
    const shareOut =
      (ousdToRedeem - ousdToRedeem * redeemInfo.ecr) /
      redeemInfo.prices.sharePrice;
  ...
  const redeem = async () => {
    try {
      const tx = await contracts.BANK.connect(signer).redeem(
        formatToDecimal(stalbeInput, 18),
        0,
        0
      );
      await tx.wait();
  ...
  const approve = async () => {
    try {
      const tx = await contracts.OUSD.connect(signer).approve(
  ...
  const RedeemButton = () => {
    if (account && userInfo) {
      if (!userInfo.ousdAllowance) {
        ...
        return (
          <RedeemBtn disabled={true}> Insufficient oUSD balance</RedeemBtn>
        );
      } else {
      ...
      return (
        <RedeemBtn onClick={() => connectWallet()}>Connect Wallet</RedeemBtn>
      );
    }
  };
    ...
    <>
      <RedeemBlockWrapper>
        <HDiv>
          ...
          <Text>
            Balance:{" "}
            {userInfo ? formattedNum(userInfo.balances.ousdBalance) : 0}
          </Text>
        </HDiv>
        <RedeemInputWrapper withSelect>
          <input
            ...
            disabled={!userInfo?.ousdAllowance}
          />
          <button
            onClick={() => handleStableInput(userInfo?.balances.ousdBalance)}
          >
            Max
          </button>
          <Divider />
        ...
        <HDiv>
          ...
          <Text>
            Balance:{" "}
            {userInfo ? formattedNum(userInfo.balances.usdcBalance) : 0}
          </Text>
        </HDiv>
        <RedeemInputWrapper>
          <input
        ...
        <HDiv>
          ...
          <Text>
            Balance: {userInfo ? formattedNum(userInfo.balances.oruBalance) : 0}
          </Text>
        </HDiv>
        <RedeemInputWrapper>
          <input
        ...
        <RedeemDataWrapper>
          ...
          <HDiv h="50px">
            ...
            <div style={{ display: "inherit", alignItems: "inherit" }}>
              ...
              <RedeemDataText bfw="500">
                <b>
                  &nbsp; = {redeemInfo ? redeemInfo.prices.stablePrice : 0}{" "}
                </b>
              </RedeemDataText>
            ...
            <div style={{ display: "inherit", alignItems: "inherit" }}>
              ...
              <RedeemDataText bfw="500">
                <b>
                  ...
                  {redeemInfo
                    ? (
                        redeemInfo.prices.collatPrice /
                        redeemInfo.prices.stablePrice
                      ).toFixed(6)
                    : 0}{" "}
                </b>
              </RedeemDataText>
          ...
          <HDiv>
            <div style={{ display: "flex", alignItems: "center", gap: "4px" }}>
              <CollectRedemtionIcon
                ratio={isMobileScreen() ? "5vw" : undefined}
              />
              <CustomSpan>Collect redemtion</CustomSpan>
            </div>
            <RedemtionBtn
              disabled={
                userInfo?.claims.collat <= 0 || userInfo?.claims.share <= 0
              }
              onClick={() => collect()}
            >
              Collect
