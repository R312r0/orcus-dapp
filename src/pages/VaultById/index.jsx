...
const VaultById = () => {
  ...
  const getBalances = async () => {
    ...
    const vaultBalance =
      (+(await vault.balanceOf(account)) / 1e18) *
      (+(await vault.getPricePerFullShare()) / 1e18);
  ...
  const MainButton = () => {
    if (!account) {
      ...
      switch (radioChoice) {
        case RADIO_CHOICE.TOKEN0:
          if (!allowances.token0) {
            return (
              <ConnectWallet onClick={() => handleApprove()}>
                Approve {pool.token0.name}
              </ConnectWallet>
            );
            ...
            return (
              <ConnectWallet disabled={true}>
                {" "}
                Insufficient {pool.token0.name} balance{" "}
              </ConnectWallet>
            );
          } else {
            return (
              <ConnectWallet onClick={() => handleDeposit()}>
                {" "}
                Deposit{" "}
              </ConnectWallet>
            );
        ...
        case RADIO_CHOICE.TOKEN1:
          if (!allowances.token1) {
            ...
            return (
              <ConnectWallet disabled={true}>
                {" "}
                Insufficient {pool.token1.name} balance{" "}
              </ConnectWallet>
            );
          } else {
            return (
      ...
      return (
        <ConnectWallet onClick={() => handleDeposit()}>Deposit</ConnectWallet>
      );
    } else if (activeDW === ACTIVE_DVS.WITHDRAW) {
      if (!allowances.vault) {
        return (
          <ConnectWallet onClick={() => handleApproveForWithdraw()}>
            Approve vault tokens
          </ConnectWallet>
        );
      } else {
        return (
          <ConnectWallet onClick={() => handleWithdraw()}>
            Withdraw
          </ConnectWallet>
        );
      }
  ...
  const handleDeposit = async () => {
    ...
    switch (radioChoice) {
      case RADIO_CHOICE.LP_TOKEN:
        tx = await vault
          .connect(signer)
          .deposit(formatToDecimal(tokenInput, 18));
        break;
      case RADIO_CHOICE.TOKEN0:
        const token0Decimals = await token0.decimals();
        tx = await router
          ...
          .beefIn(
            ...
            0,
            token0.address,
            formatToDecimal(tokenInput, token0Decimals)
          );
        break;
      case RADIO_CHOICE.TOKEN1:
        const token1Decimals = await token1.decimals();
        tx = await router
          ...
          .beefIn(
            ...
            0,
            token1.address,
            formatToDecimal(tokenInput, token1Decimals)
          );
        break;
      case RADIO_CHOICE.ASTR:
        tx = await router
          .connect(signer)
          .beefInETH(pool.vaultAddress, 0, {
            value: formatToDecimal(tokenInput, 18),
          });
        break;
      default:
        return;
  ...
  const handleWithdraw = async () => {
    ...
    const vaultLpMultiplier = formatFromDecimal(
      (await vault.getPricePerFullShare()).toString(),
      18
    );
    ...
    switch (radioChoice) {
      case RADIO_CHOICE.LP_TOKEN:
        tx = await router
          ...
          .beefOut(
            vault.address,
            formatToDecimal(tokenInput / +vaultLpMultiplier, 18)
          );
        break;
      case RADIO_CHOICE.TOKEN0:
        tx = await router
    ...
    <VidWrapper>
      {pool ? (
        <>
          <VidTopBar>
            <div
              style={{ display: "flex", gap: "0.8vw", alignItems: "center" }}
            >
              <ArrowBack
            ...
            <div>
              <Font fs={isMobileScreen() ? "12px" : "0.72vw"} fw="300">
                Platform:
              </Font>
              <Font
                fs={isMobileScreen() ? "12px" : "0.72vw"}
                fw="500"
                color="#4F4F4F"
              >
                {" "}
            ...
            <WhiteBorderBar>
                ...
                <div>
                  ...
                  <div>
                    <Font
                      fw="500"
                      color="#828282"
                      fs={isMobileScreen() ? "14px" : "0.83vw"}
                    >
                      $
                    </Font>
                    <Font fs={isMobileScreen() ? "14px" : "0.83vw"}>
                      {formattedNum(pool.tvlLocal)}
                    </Font>
                  </div>
                </div>
              </WhiteBorderItem>
              ...
              <WhiteBorderItem bg="#E4DDEF">
                <div>
                  ...
                  <div>
                    <Font fw="500" fs={isMobileScreen() ? "14px" : "0.83vw"}>
                      {formattedNum(pool.apr.toFixed(2))}%
                    </Font>
                  </div>
                </div>
              </WhiteBorderItem>
              ...
              <WhiteBorderItem bg="#D5ECD8">
                <div>
                  ...
                  <div>
                    <Font fw="500" fs={isMobileScreen() ? "14px" : "0.83vw"}>
                      {formattedNum((pool.apr / 365).toFixed(2))}%
                    </Font>
                  </div>
                </div>
              </WhiteBorderItem>
              ...
              <WhiteBorderItemLarge>
                ...
                >
                  <Font fs={isMobileScreen() ? "12px" : "0.72vw"}>
                    Your deposit
                  </Font>
                  <div>
                    <Font fw="500" fs={isMobileScreen() ? "14px" : "0.83vw"}>
                      {balances ? fromExponential(+balances.deposited) : null}
                    </Font>
                  </div>
                  <div style={{ marginTop: "-0.36vw" }}>
                    <Font
                      fw="300"
                      color="#4F4F4F"
                      fs={isMobileScreen() ? "14px" : "0.62vw"}
                    >
                      {balances ? fromExponential(+pool.deposited.usd) : null}
                    </Font>
                  </div>
                </div>
                <VDivider />
                <div
                  style={{
                    width: "100%",
                    paddingLeft: isMobileScreen() ? "20px" : "1.71vw",
                  }}
                >
          ...
          <VIDLayout>
            <VIDLeftColumn>
              <VidBlock height={"21.26vw"}>
                ...
                <HDivider />
                <LinksRow>
                  <a href={pool.platform.website}>
                    Website
                    <OutLinkIcon ratio={isMobileScreen() ? "4vw" : "0.93vw"} />
                  </a>
                  <a href={pool.platform.telegram}>
                    Telegram
                    <OutLinkIcon ratio={isMobileScreen() ? "4vw" : "0.93vw"} />
                  </a>
                  <a href={pool.platform.twitter}>
                    Twitter
                    <OutLinkIcon ratio={isMobileScreen() ? "4vw" : "0.93vw"} />
                  </a>
                </LinksRow>
                ...
                <>
                  ...
                  >
                    <VidBlockHeader>
                      ...
                      <div
                        style={{
                          marginTop: isMobileScreen() ? "8px" : "",
                          marginBottom: isMobileScreen() ? "8px" : "",
                        }}
                      >
                        <Font
                          fw="500"
                    ...
                    <LinksRow>
                      {pool.token0.website ? (
                        <a href={pool.token0.website}>
                          Website
                          <OutLinkIcon
                            ratio={isMobileScreen() ? "4vw" : "0.93vw"}
                          />
                        </a>
                      ) : null}
                      <a
                        href={`https://blockscout.com/astar/address/${pool.token0.address}/transactions`}
                      >
                        Token Contract
                        <OutLinkIcon
                          ratio={isMobileScreen() ? "4vw" : "0.93vw"}
                  ...
                  <VidBlock height={"8.96vw"}>
                    ...
                    <LinksRow>
                      ...
                      <a
                        href={`https://blockscout.com/astar/address/${pool.token1.address}/transactions`}
                      >
                        Token Contract
                        <OutLinkIcon
                          ratio={isMobileScreen() ? "4vw" : "0.93vw"}
                    ...
                    <VidBlockText mt="1.25vw">
                      {pool.token1.description}
                    </VidBlockText>
                  </VidBlock>{" "}
                </>
              )}
              ...
              >
                <VidBlockHeader>
                  <DWContainer>
                    <DWButton
                      data-value="Deposit"
                      onClick={updateActiveDW}
                      active={activeDW === "Deposit"}
                    >
                      Deposit
                    </DWButton>
                    <DWButton
                ...
                <VidBlockText mt="0.83vw">
                  Deposit your LP or ZAP⚡ in {pool.token0.name} or{" "}
                  {pool.token1.name}
                </VidBlockText>
                <VidBlockText mt="0.83vw">WALLET:</VidBlockText>
                <Fieldset>
                  <Field>
                    <FieldInput
                      type="radio"
                      checked={radioChoice === RADIO_CHOICE.LP_TOKEN}
                      name={pool.lpName}
                      onClick={() => setRadioChoice(RADIO_CHOICE.LP_TOKEN)}
                    />
                  ...
                  <Field>
                    <FieldInput
                      type="radio"
                      checked={radioChoice === RADIO_CHOICE.TOKEN0}
                      name={pool.token0.name}
                      onClick={() => setRadioChoice(RADIO_CHOICE.TOKEN0)}
                    />
                  ...
                  <Field>
                    <FieldInput
                      type="radio"
                      checked={radioChoice === RADIO_CHOICE.TOKEN1}
                      name={pool.token1.name}
                      onClick={() => setRadioChoice(RADIO_CHOICE.TOKEN1)}
                    />
                    {activeDW === ACTIVE_DVS.WITHDRAW ? (
                      <>
                        {pool.token1.name === "WASTR" ? (
                          <label> ASTR </label>
                        ) : (
                    ...
                    <>
                      {activeDW !== ACTIVE_DVS.WITHDRAW ? (
                        <Field>
                          <FieldInput
                            type="radio"
                            checked={radioChoice === RADIO_CHOICE.ASTR}
                            name={"ASTR"}
                            onClick={() => setRadioChoice(RADIO_CHOICE.ASTR)}
                          />
                          <label>ASTR</label>
                        </Field>
                      ) : null}
                ...
                <AddBuyContainer>
                  <AddButton href={pool.addLpLink} target={"_blank"}>
                    Add Liquidity
                  </AddButton>
                  <BuyButton href={pool.buyLink} target={"_blank"}>
                    Buy Token
                  </BuyButton>
                </AddBuyContainer>
                <InputContainer>
                  ...
                  >
                    <LogoIconBlack
                      ratio={isMobileScreen() ? "5vw" : "0.94vw"}
                    />
                  </div>
                  <Input
                    placeholder="0.0"
                ...
                <VidBlockText mt={isMobileScreen() ? "24px" : "2.03vw"}>
                  ...
                  <Font
                    fs={isMobileScreen() ? "10px" : "0.62vw"}
                    color="#828282"
                    fw="400"
                  >
                    Performance fees are already subtracted from the displayed
                    APY.
                  </Font>
