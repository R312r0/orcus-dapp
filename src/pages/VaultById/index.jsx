...
const VaultById = () => {
    ...
    const init = (vault) => {
        const tokens = vault.Tokens.map(token => ({...token, contract: new ethers.Contract(token.address, ERC20_ABI, signer)}))

        tokens.push({name: "VAULT", decimals: 18, address: vault.vaultAddress, contract: new ethers.Contract(vault.vaultAddress, ERC20_ABI, signer)});
        tokens.push({name: "LP", decimals: 18, address: vault.lpAddress, contract: new ethers.Contract(vault.lpAddress, ERC20_ABI, signer)})

        const vaultContract = new ethers.Contract(vault.vaultAddress, VAULT_ABI, signer);
        const routerContract = new ethers.Contract(vault.Project.routerAddress, ROUTER_ABI, signer);

        return {...vault, vaultContract, routerContract, tokenContracts: tokens};
    }
    ...
    const getAllowances = async (vaultLoc) => {
        ...
        const {tokenContracts} = vaultLoc;
        const allowances = tokenContracts.map(async (token) => {

            const statement = token.name === "LP";
            const res = await token.contract.allowance(account, statement ? vault.vaultAddress : vault.Project.routerAddress);

            return {name: token.name, allowance: res > 0};

        })

        setAllowances(await Promise.all(allowances));
    }
    ...
    const getBalances = async (vaultLoc) => {
        ...
        const astarBalance = +(await signer.getBalance()) / 1e18;

        const balances = await Promise.all(tokenContracts.map(async (token) => {
            const bal = +(await token.contract.balanceOf(account)) / 10**token.decimals;
            return {name: token.name, bal};
        }))

        balances.push({name: "ASTAR", bal: astarBalance})
...
{/*    TODO: make a map of vault.Tokens => and field with token choises will be there maybe rewrite some logic*/}
