...
import SRS_ABI from "../abis/SRS.json";
import GAGUE_ABI from "../abis/GaugeController.json";
...
export const BlockchainContextProvider = ({ children }) => {
  ...
  useEffect(() => {
    if (account) {
      createSigner().then((res) => setSigner(res));
    }
  }, [active, account]);
  ...
  const getLiquidityPrices = async () => {
    ...
    const oruPrice =
      formatFromDecimal(+oruUsdcRes[0], 6) /
      formatFromDecimal(+oruUsdcRes[1], 18);
    const oruUsdcLiq =
      +oruUsdcRes[0] / 1e6 + (+oruUsdcRes[1] / 1e18) * oruPrice;
    ...
    const ousdUsdcLiq =
      ousdPrice * (+ousdUsdcRes[0] / 1e18) + +ousdUsdcRes[1] / 1e6;

    // OUSD-ORU liquidity
    // TODO: Add liquidity for this pair.
    const oruOusdRes = await OUSD_ORU.getReserves();
    const oruOusdLiq =
      ousdPrice * (+oruOusdRes[0] / 1e18) + oruPrice * (+oruOusdRes[1] / 1e18);
  ...
  const init = async () => {
    ...
    const tokens = {
      ORU: new ethers.Contract(
        CONTRACT_ADDRESSES.ORU,
        OrcusERC20_ABI,
        readProvider
      ),
      OUSD: new ethers.Contract(
        CONTRACT_ADDRESSES.OUSD,
        OrcusERC20_ABI,
        readProvider
      ),
      ...
      WASTR: new ethers.Contract(
        CONTRACT_ADDRESSES.WASTR,
        ERC20_ABI,
        readProvider
      ),
      XORU: new ethers.Contract(
    ...
    const pairs = {
      ORU_USDC: new ethers.Contract(
        CONTRACT_ADDRESSES.ORU_USDC,
        UNISWAP_PAIR_ABI,
        readProvider
      ),
      OUSD_USDC: new ethers.Contract(
        CONTRACT_ADDRESSES.OUSD_USDC,
        UNISWAP_PAIR_ABI,
        readProvider
      ),
      OUSD_ORU: new ethers.Contract(
        CONTRACT_ADDRESSES.OUSD_ORU,
        UNISWAP_PAIR_ABI,
        readProvider
      ),
    };
    ...
    const contracts = {
      ...
      BANK_SAFE: new ethers.Contract(
        CONTRACT_ADDRESSES.BANK_SAFE,
        BANK_SAFE_ABI,
        readProvider
      ),
      ORU_STAKE: new ethers.Contract(
        CONTRACT_ADDRESSES.ORU_STAKE,
        ORU_STAKE_ABI,
        readProvider
      ),
      PRICE_ORACLE: new ethers.Contract(
        CONTRACT_ADDRESSES.PRICE_ORACLE,
        PRICE_ORACLE_ABI,
        readProvider
      ),
      OUSD_USDC_ORACLE: new ethers.Contract(
        CONTRACT_ADDRESSES.OUSD_USDC_ORACLE,
        TWAP_ORACLE_ABI,
        readProvider
      ),
      MASTER_CHEF: new ethers.Contract(
        CONTRACT_ADDRESSES.MASTER_CHEF,
        MASTER_CHEF_ABI,
        readProvider
      ),
      ...
      OUSD_METAPOOL: new ethers.Contract(
        CONTRACT_ADDRESSES.OUSD_METAPOOL,
        OUSD_METAPOOL_ABI,
        readProvider
      ),
      SRS: new ethers.Contract(CONTRACT_ADDRESSES.SRS, SRS_ABI, readProvider),
      GAGUE_CONTROLLER: new ethers.Contract(
        CONTRACT_ADDRESSES.GAGUE_CONTROLLER,
        GAGUE_ABI,
        readProvider
      ),
      SRS_LP_TOKEN: new ethers.Contract(
        CONTRACT_ADDRESSES.SRS_LP_TOKEN,
        LP_TOKEN_SRS,
        readProvider
      ),
      GAGUE_OUSD: new ethers.Contract(
        CONTRACT_ADDRESSES.GAGUE_OUSD,
        GAGUE_OUSD_ABI,
        readProvider
      ),
